...
void Context::initializeExtensions(glProc (*getProcAddress)(const char*)) {
    if (const char* extensions =
            reinterpret_cast<const char*>(MBGL_CHECK_ERROR(glGetString(GL_EXTENSIONS)))) {

        auto fn = [=](
            std::initializer_list<std::pair<const char*, const char*>> probes) -> glProc {
            for (auto probe : probes) {
                if (strstr(extensions, probe.first) != nullptr) {
                    return getProcAddress(probe.second);
                }
            }
            return nullptr;
        };
